...
# Imports
from fairseq.data.indexed_dataset import MMapIndexedDatasetBuilder, MMapIndexedDataset
...
def transfer(item: str) -> None:
    ...
    if len(mid_files) == 0: return
    # Copy file
    for i in range(len(mid_files)):
        shutil.copy(
            path + mid_files[i], 
            f"{MIDI_TOK_LOC}/{mid_files[i]}"
        )
...
if __name__ == "__main__":
    ...
    # test
    data = MMapIndexedDataset(f'{FINAL_LOC}/valid')
    print(len(data))
    data = MMapIndexedDataset(f'{FINAL_LOC}/train')
    print(len(data))
