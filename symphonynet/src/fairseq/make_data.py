...
def mp_handler(raw_data, str2int, output_file, ratio, sample_len_max, num_workers=WORKERS):
    ...
    if output_file.split('/')[-1] == 'train':
        ...
        with open('/home/blherre4/VIVY/VIVYNet/decoder/symphony_net/src/fairseq/mea_cnt_dis.txt', 'w') as f:
            for k, v in sorted(mea_cnt_dis.items()):
                f.write(f'{k*10} {v}\n')
        with open('/home/blherre4/VIVY/VIVYNet/decoder/symphony_net/src/fairseq/mea_len_dis.txt', 'w') as f:
...
if __name__ == '__main__':
    ...
    print('config.sh: ')
    subprocess.run(['pwd'])
    with open('/home/blherre4/VIVY/VIVYNet/decoder/symphony_net/config.sh', 'r') as f:
        for line in f:
            line = line.strip()
            if len(line) == 0:
                break
    ...
    raw_data_path = f'/home/blherre4/VIVY/VIVYNet/decoder/symphony_net/data/preprocessed/{raw_corpus}.txt'
    output_dir = f'/home/blherre4/VIVY/VIVYNet/decoder/symphony_net/data/model_spec/{model_name}_hardloss{map_meta_to_pad}/'
