...
def gen_one(model, src_input, prime_nums, MAX_LEN = 4090, MIN_LEN = 0):
    ...
    with torch.no_grad():
        ...
        for item, next_item in zip(prime[:-1], prime[1:]):
            ...
            (e,d,t,ins), memo = get_next(model, src_input, item, memo, has_prime=True)
            cur_rel_pos, cur_mea = calc_pos(next_item[0], cur_rel_pos, cur_mea)
            ins_list.append(ins)


        (e,d,t,ins), memo = get_next(model, src_input, prime[-1], memo, has_prime=False)
