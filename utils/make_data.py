...
def midi_binarize(train_ratio: float) -> None:
    ...
    # create bin folder and place the dictionary with the corresponding contenting into it
    os.makedirs(output_dir, exist_ok=True)
    with open(output_dir + 'dict.y.txt', 'w') as f:
    ...
    for type in range(RATIO):
        ...
        with open(output_dir + 'vocab_%d.json'%type, 'w') as f:
            json.dump({i:v for i,v in enumerate(sub_vocab)}, f)
    with open(output_dir + 'ori_dict.json', 'w') as f:
    ...
    for mode in splits:
        print(mode)
        mp_handler(splits[mode], voc_to_int, output_dir + f'{mode}.x-y.y', ratio=RATIO, sample_len_max=SAMPLE_LEN_MAX)
